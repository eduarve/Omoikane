 ...
 */
public class TraspasoEntranteController implements Initializable {

    public static final Logger logger = Logger.getLogger(TraspasoEntranteController.class);

    TraspasoEntrantePropWrapper modelo;
    ...
    @FXML public void onImprimir(ActionEvent actionEvent) {
        ...
        try {
            for (ItemTraspasoEntrante itemTraspasoEntrante : modelo._traspasoEntrante.getItems()) {
                HashMap<String, Object> mapa = new HashMap<>();
                mapa.put("codigo", itemTraspasoEntrante.getCodigo());
                mapa.put("descripcion", itemTraspasoEntrante.getNombre());
                mapa.put("cantidad", itemTraspasoEntrante.getCantidad());
                //mapa.put("existencia", itemTraspasoEntrante.getStockDB());
                //mapa.put("precioUnitario", itemTraspasoEntrante.getPrecioPublico());
                //mapa.put("costoUnitario", itemTraspasoEntrante.getCostoUnitario());
                    ...
                    .title(
                            cmp.horizontalList().add(
                                cmp.verticalList().add(
                                    cmp.text("Recibo de mercancía").setStyle(boldCenteredStyle2).setWidth(300),
    ...
    private void aplicarTraspasoToModel() {
        ...
        for (ItemTraspasoPropWrapper itemTraspasoPropWrapper : modelo.getItems()) {
            ...
            Articulo articulo = stockIssuesLogic.increaseStock(a.getIdArticulo(), itemTraspasoPropWrapper.cantidadProperty().get() );
        ...
        repo.saveAndFlush(modelo._traspasoEntrante);
    ...
    // -------------------------------------------------
    /**
     * Lee archivo json en el path de dropbox que contiene los metadatos del traspaso que tiene la UID especificada
     */
    // -------------------------------------------------
    private void recibirTraspaso(String uid) {
        ObjectMapper mapper = new ObjectMapper(); // can reuse, share globally
        try {
            TraspasoSaliente ts = new TraspasoSaliente();
            File json = new File(Principal.dropboxPath+uid+".json");
            ts = mapper.readValue(json, TraspasoSaliente.class);

            // Por defecto lo escribo directo a la GUI. Sin embargo, lo correcto sería escribirlo al modelo...
            this.uid.textProperty().setValue( ts.getUid() );
            almacenOrigen.textProperty().setValue( ts.getAlmacenOrigen() );
            almacenDestino.textProperty().setValue( ts.getAlmacenDestino() );
            notasTextArea.appendText(" ------ Notas del almacén origen (emisor) ------ \n"+ts.getNotas());

            // ----------------- Filas / items -----------------
            StringBuilder errores = new StringBuilder();
            Integer encontrados = 0, noEncontrados = 0;

            for(omoikane.inventarios.traspasoSaliente.ItemTraspasoSaliente itemTraspasoSaliente : ts.getItems()) {
                String codigo = itemTraspasoSaliente.getCodigo();
                BigDecimal cantidad = itemTraspasoSaliente.getCantidad();
                Articulo a = getArticulo(codigo);
                if (a == null)
                    {
            ...
            logger.error("Error al leer traspaso", e);
        }
    }
    ...
    // -------------------------------------------------
    /**
     * Evento del botón ", envuelve el método importar en un nuevo hilo de swing
     */
    // -------------------------------------------------
    @FXML public void onRecibirTraspasoAction(ActionEvent actionEvent) {
        TextInputDialog dialog = new TextInputDialog("");
        dialog.setTitle("Importación de traspaso");
        dialog.setHeaderText("Recibir traspaso entrante de mercancía");
        dialog.setContentText("Por favor indica el UID del traspaso saliente a recibir: ");

        Optional<String> result = dialog.showAndWait();
        result.ifPresent(uid -> recibirTraspaso(uid));
    }
    ...
    public void addItemConteo(Articulo capturaArticulo, BigDecimal conteo) throws Exception {
        ...
        if(indice.containsKey(capturaArticulo.getIdArticulo())) {
            ...
            ItemTraspasoEntrante newItemBean = new ItemTraspasoEntrante(codigo, descripcion, conteo, ultimoCosto);
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        assert agregarButton != null : "fx:id=\"agregarButton\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        assert codigoCol != null : "fx:id=\"codigoCol\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        assert codigoTextField != null : "fx:id=\"codigoTextField\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        assert cantidadCol != null : "fx:id=\"cantidadCol\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        assert conteoTextField != null : "fx:id=\"conteoTextField\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        assert descripcionLabel != null : "fx:id=\"descripcionLabel\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        assert fechaLabel != null : "fx:id=\"fechaLabel\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        assert idLabel != null : "fx:id=\"idLabel\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        assert itemsTable != null : "fx:id=\"itemsTable\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        assert nombreProductoCol != null : "fx:id=\"nombreProductoCol\" was not injected: check your FXML file 'TraspasoEntranteView.fxml'.";
        ...
        codigoTextField.setOnKeyReleased(new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent event) {
                ...
                if(event.getCode() == KeyCode.F1)
                    new MostrarCatalogoHandler(TraspasoEntranteController.this).handle();
            }
        });
        ...
        ActionsCell(final TableView table) {
            ...
            delButton.setOnAction(new EventHandler<ActionEvent>() {
                @Override public void handle(ActionEvent actionEvent) {
                    ...
                    TraspasoEntranteController.this.indice.remove(item.getBean().getArticulo().getIdArticulo());
            ...
            protected Void call() throws Exception {
                ...
                Platform.runLater(new Runnable() {
                    ...
                    public void run() {
                        ...
                        TraspasoEntranteController.this.capturaArticulo = finalResultado;
                    }
                });
    ...
    public void initModel(TraspasoEntrante traspasoEntrante) {
        ...
        if(modelo != null) modelo.getUid().unbind();
        ...
        //------------// Cargar modelo //--------------
        modelo = loadOrCreateModel(traspasoEntrante);
        ...
        almacenOrigen.setText( traspasoEntrante != null ? traspasoEntrante.getAlmacenOrigen() : "" );  //AlmacenOrigen
        almacenOrigen.textProperty().set( modelo.getAlmacenOrigen().get() );
        modelo.getAlmacenOrigen().bind(almacenOrigen.textProperty());
        almacenDestino.setText( traspasoEntrante != null ? traspasoEntrante.getAlmacenDestino() : "" );  //AlmacenDestino
        almacenDestino.textProperty().set( modelo.getAlmacenDestino().get() );
        modelo.getAlmacenDestino().bind(almacenDestino.textProperty());
        notasTextArea.setText( traspasoEntrante != null ? traspasoEntrante.getNotas() : "" );  //Notas
        notasTextArea.textProperty().set(modelo.getNotas().get());
        modelo.getNotas().bind(notasTextArea.textProperty());
        uid.setText( traspasoEntrante != null ? traspasoEntrante.getAlmacenOrigen() : "" );  //UID
        uid.textProperty().set(modelo.getUid().get());
        modelo.getUid().bind(uid.textProperty());
     ...
     * */
    @Transactional
    private TraspasoEntrantePropWrapper loadOrCreateModel(TraspasoEntrante traspasoEntrante) {

        if(traspasoEntrante == null)
            traspasoEntrante = repo.findByCompletado(false);
        if(traspasoEntrante == null) {
            traspasoEntrante = new TraspasoEntrante();
            traspasoEntrante.setFecha( new Date() );
            traspasoEntrante = repo.saveAndFlush(traspasoEntrante);
        }

        TraspasoEntrantePropWrapper conteoInventarioPropWrapper = new TraspasoEntrantePropWrapper(traspasoEntrante);

        return conteoInventarioPropWrapper;
    }

    private TraspasoEntrantePropWrapper loadOrCreateModel() {
        TraspasoEntrantePropWrapper conteoInventarioPropWrapper = loadOrCreateModel(null);
    ...
    public void persistModel() throws Exception {
        ...
        transactionTemplate.execute(new TransactionCallbackWithoutResult()
        {
            ...
            {
                entityManager.merge(modelo._traspasoEntrante);
                entityManager.flush();
            }
        });
    ...
    private Task<Void> deleteModel() {
        Task deleteTask = new Task<Void>() {
            @Override
            protected Void call() throws Exception {
                repo.delete(modelo._traspasoEntrante);
                return null;
            }
        };
        return deleteTask;
